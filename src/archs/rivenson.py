# Replication study for lymphoma dataset: a deep fully convolutional architecture
...
# Custom upsampling layer (Rivenson et al - Suppl Fig. 1 & 2)
    ...
    def call(self, x):
        ...
        for batch in range(BATCH_SIZE):
            ...
            for ch in range(0, input_ch, 4):
                ...
                x_s2 = K.stack([x[batch, :, :,  ch+2], x[batch, :, :, ch+3]], -2)
...
u0 = K.reshape(r0, [BATCH_SIZE, int(r0.shape[1]), int(r0.shape[2]), int(r0.shape[3])])
...
out = Conv2D(2, (3, 3), activation='relu', padding='same')(p0)
dense_out = Dense(8)(out)
...
# load npz
data_folder = '../../data/'
train_npz = np.load(data_folder + 'ds-lymphoma-training.npz')
test_npz = np.load(data_folder + 'ds-lymphoma-test.npz')
